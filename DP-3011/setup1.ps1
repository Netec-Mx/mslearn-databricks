...
$supported_regions = "centralus","southcentralus","eastus","eastus2","westus","westus2","westcentralus"
...
while ($stop -ne 1){
    ...
    $skuOK = 1
    $skus = Get-AzComputeResourceSku $Region | Where-Object {$_.ResourceType -eq "VirtualMachines" -and $_.Name -eq "Standard_D4ds_v4"}
    if ($skus.length -gt 0)
    {
        $r = $skus.Restrictions
        if ($r -ne $null)
    ...
    if ($skuOK -eq 1)
        {
            $quota = @(Get-AzVMUsage -Location $Region).where{$_.name.LocalizedValue -match 'Standard Ddsv4 Family vCPUs'}
